...
<body class="transition-colors">
  ...
  <main>
    <section id="erro" class="bg-modal shadow-lg shadow-stone-700 w-2/5 absolute top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 rounded-lg border border-stone-500 max-lg:w-full flex justify-center items-center flex-col">
      <section class="justify-center flex items-center pt-4">
        <span class="font-bold text-2xl max-lg:text-xl text-padrao">USUARIO NÃO ENCONTRADO</span>
      </section>
      <p class="text-xl mt-11 text-center w-3/4 text-padrao">
        Desculpe, mas não existe Usuario cadastrado com essa informação!
      </p>
      <section class="flex items-center mt-10 gap-2 pb-7">
        <button class="font-semibold hover:bg-red-700 bg-modal-button hover:text-white transition-colors shadow-xl w-32 h-10 border-red-700 border text-padrao " onclick="window.history.back()">
          Ok
        </button>
      </section>
    </section>
